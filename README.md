...
- Answers are produced by **sympy** and stored as sympy's LaTeX output. The
  stored form is normalized for readability (`generate._present`): rational
  results get an expanded numerator over a factored denominator (textbook form),
  while non-rational results keep `simplify`'s output (so `sin·cos → cos(2x)`
  etc. survive). `sp.simplify` alone is avoided because it can leave rational
  answers in raw quotient-rule shape (e.g. `-x² + 2x(x-1) - 1`). All forms are
